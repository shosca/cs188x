...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        for i in range(len(self.corners)):
            state.append(not self.goal[i])

        return self.startingPosition + (tuple(state),)

    def isGoalState(self, state):
        "Returns whether this search state is a goal state of the problem"
        x, y, corners = state
        isGoal = corners == self.goal
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not self.walls[nextx][nexty]:
                ...
                nextc = list(corners)
                for i in range(len(self.corners)):
                    if not nextc[i] and self.corners[i] == nextpos:
                        nextc[i] = True

                nextState = nextpos + (tuple(nextc),)
...
def cornersHeuristic(state, problem):
    ...
    while len(cornerlist) > 0:
        distances = []  # calculate the distances of all nodes from all other nodes
        for i in range(len(cornerlist)):
            xy2 = cornerlist[i]
            m = min(xy1, xy2)
            x = max(xy1, xy2)
            p = (m, x)
            if p not in problem.heuristicInfo:
                d1 = util.manhattanDistance(xy1, xy2)
                d2 = mazeDistance(m, x, problem.startingGameState)
                problem.heuristicInfo[p] = min(d1, d2)

            distances.append((problem.heuristicInfo[p], xy2))

        distances.sort()
        mindist, xy1 = distances.pop(0)
        distance += mindist
        cornerlist.remove(xy1)
...
def foodHeuristic(state, problem):
    ...
    for i in range(len(foodsList)):
        ...
        p = (m, x)
        if p not in problem.heuristicInfo:
            problem.heuristicInfo[p] = mazeDistance(m, x, problem.startingGameState)
            # problem.heuristicInfo[p] = util.manhattanDistance(m, x)

        d = problem.heuristicInfo[p]
        if disttoclosest == None or disttoclosest > d:
            disttoclosest = d
    ...
    if disttoclosest == None:
        disttoclosest = 0

    return disttoclosest + distxtremes
...
class WholeFoodzSearchProblem(AnyFoodSearchProblem):
    def isGoalState(self, state):
        import pdb
